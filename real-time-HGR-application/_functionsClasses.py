...
if hasattr(args, "verbose"): 
    print(e_details, end=f"\n{'-'*15}\n")
    if args.verbose: os._exit(os.EX_OK)
# -----------------------------------------------
...
opt_InitEpochs = {"2d": 20, "3d": 20}  # @E21/22
opt_InitItrFinetune = {"2d": True, "3d": False}  # @E21/22
opt_ItrScalingSizes = {"2d": [224, 224, 276, 328], "3d": [276, 328, 380]}  # @E21/22
itr_scl_sizes = opt_ItrScalingSizes[args.nd] if args.itr_scl_sizes == ["<OPT>"] else args.itr_scl_sizes
liveStreamVocab = ["Swipe+", "SwipeDown", "SwipeLeft", "SwipeRight", "SwipeUp", "SwipeV", "SwipeX"]
# -----------------------------------------------
...
class tunerImagesBW(fastuple):
    @classmethod
    def create(cls, X):
        # assert len(Xy) == 2, ">> InputError: expected list/tuple if X and y data!!"  # [DEPRECATED!]
        ...
        for _idx, (_img, _stt_band) in enumerate(zip(imgs, stt_bands)):
            _img[_stt_band:_stt_band+l_band, :] = [255, 255, 255]
            imgs[_idx] = _img
...
def tunerImagesBWBlock():
    return TransformBlock(type_tfms=tunerImagesBW.create, batch_tfms=IntToFloatTensor)
...
def multiOrientationDataLoader(ds_directory, bs, img_size, shuffle=True, return_dls=True, ds_valid="valid", preview=False, _e_seed_worker=None, _e_repr_gen=None):
    tfms = aug_transforms(
...
def Cleaner(target=None):
    ...
    for _f in Path(f"./.sources").iterdir():
        if _f.is_dir() and not(list(_f.iterdir())) and not(_f.name.startswith("DHG1428")):
            _cleanup(_f)
# -----------------------------------------------
...
def getLR(learn, show_plot=False):
    return i_LRFinder(learn, show_plot=show_plot).valley
...
def iterativeScaling(query_tuple, nd, bs, img_sz, eps, pct_start=None, finetune=False):
    ...
    learn.dls = multiOrientationDataLoader(deets.ds_directory, bs=args.bs, img_size=img_sz)
...
class end2endTunerModel(Module):
    def __init__(self, archMultiVOs, archTuner, tuner_img_sz=224, debug=False):
        self.tuner_img_sz = tuner_img_sz
        self.debug = debug

        self.multiVOsBody = create_body(arch=archMultiVOs, cut=None)
        self.multiVOsHead = create_head(nf=num_features_model(self.multiVOsBody), n_out=args.n_classes)
        self.tunerBody = create_body(arch=archTuner, cut=None)
        self.tunerHead = create_head(nf=num_features_model(self.tunerBody), n_out=args.n_classes)
    ...
    def forward(self, X):
        ...
        tnr_X = self.batch_tuner_images(mvo_preds)

        tnr_ftrs = self.tunerBody(tnr_X)
        tnr_ftrs = self.tunerHead(tnr_ftrs)

        if self.debug:
            print(f">> {tnr_X.shape= } | {mvo_preds.shape= } | {tnr_ftrs.shape= }")
            self.debug = False #; os._exit(os.EX_OK)

        return [*mvo_ftrs, tnr_ftrs]

    def batch_tuner_images(self, preds):
        batch = []
        preds = preds.detach().cpu().numpy()

        for _idx, _X in enumerate(preds):
            if self.debug and (_idx == 0): print(f"@batch_tuner_images: {_X.shape=} \n>> {_X=}")

            img = tunerImagesBW.create(_X)
